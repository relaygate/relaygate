...
func TestSeedDefaultsCreatesSkeletonAndCopiesTemplates(t *testing.T) {
	...
	for _, d := range runtimeDataDirs {
		if st, err := os.Stat(filepath.Join(root, "data", d)); err != nil || !st.IsDir() {
			t.Fatalf("missing data/%s: %v", d, err)
		}
	}
	res := mustRead(t, filepath.Join(root, "data", "resources.yaml"))
	if !strings.Contains(res, "gateway_name: example") {
		t.Fatalf("resources not seeded: %q", res)
	}
	inv := mustRead(t, filepath.Join(root, "data", "inventory", "gateways.env"))
...
func TestSeedDefaultsDoesNotOverwriteExisting(t *testing.T) {
	...
	mustWrite(t, filepath.Join(root, "data", "resources.yaml"), "from: user\n")
	mustWrite(t, filepath.Join(root, "data", "inventory", "gateways.env"), "from=user\n")

	if err := SeedDefaults(root, false); err != nil {
		t.Fatal(err)
	}
	if got := mustRead(t, filepath.Join(root, "data", "resources.yaml")); got != "from: user\n" {
		t.Fatalf("resources overwritten: %q", got)
	}
	if got := mustRead(t, filepath.Join(root, "data", "inventory", "gateways.env")); got != "from=user\n" {
...
func TestSeedDefaultsResetDefaults(t *testing.T) {
	...
	if got := mustRead(t, filepath.Join(root, "data", "resources.yaml")); got != "from: example\n" {
		t.Fatalf("resources not reset: %q", got)
	}
	if got := mustRead(t, filepath.Join(root, "data", "inventory", "gateways.env")); got != "from=example\n" {
...
func TestSeedDefaultsMissingExampleIsOKWhenNotReset(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "data", "resources.yaml")); !os.IsNotExist(err) {
...
func TestSeedDefaultsResetRequiresExample(t *testing.T) {
	root := t.TempDir()
	if err := SeedDefaults(root, true); err == nil {
		t.Fatal("expected error when reset without templates")
	}
}
