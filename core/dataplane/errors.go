...
func UserFacingError(err error) string {
	...
	case strings.Contains(low, "ack timeout"), strings.Contains(msg, "ACK timeout"):
		if strings.Contains(low, "rollback") || strings.Contains(msg, "回滚") {
			return "Envoy 未在时限内确认新配置，且回滚上一快照失败。请立即执行「硬重启应用」（reload --hard），并检查本机 Panel/xDS 与 Envoy 是否正常。"
		}
		return "Envoy 未在时限内确认新配置，已尝试回滚上一快照。请检查本机 xDS/Panel 与 Envoy；仍异常时执行 reload --hard。"
	...
	case strings.Contains(low, "no previous snapshot"), strings.Contains(msg, "无上一版快照"):
		return "无法回滚上一快照（尚无历史版本）。请执行 reload --hard 恢复，并确认 Panel/xDS 已正常启动。"
	case strings.Contains(low, "mgmt") && strings.Contains(low, "not listening"):
		return "本机 xDS 控制面未就绪（ADS 端口被占用但管理口未监听）。请启动 Panel 或执行 relaygate xds serve。"
	case strings.Contains(msg, "xds ADS"), strings.Contains(low, "xds:"):
		return "本机 xDS 控制面不可用。请确认 Panel 或 relaygate xds serve 已运行，或执行 reload --hard。"
	case strings.Contains(msg, "bootstrap"), strings.Contains(msg, "未迁移"):
		return "磁盘 Envoy 配置尚未迁移到热更新模式。请按迁移说明执行一次 reload --hard（见 docs/xds-migrate.md）。"
	...
	case strings.Contains(msg, "standby"):
		return "当前节点为只读（standby），禁止写操作。请到 Management Primary 执行。"
