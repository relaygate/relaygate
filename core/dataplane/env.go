...
// Env is an alias for config.Env for call-site convenience within dataplane.
type Env = config.Env
...
func requireEnvFile(root string) error {
	...
	if _, err := os.Stat(path); err != nil {
		return fmt.Errorf("缺少 .env，请先: relaygate setup 或 cp packaging/shared/env.example .env")
	}
	return nil
}
