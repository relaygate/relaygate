...
// RunCmdCaptureEnv is RunCmdCapture with additional environment variables (e.g. SSHPASS).
func RunCmdCaptureEnv(root string, extraEnv []string, name string, args ...string) (string, error) {
	var buf bytes.Buffer
	cmd := exec.Command(name, args...)
	cmd.Dir = root
	cmd.Stdout = &buf
