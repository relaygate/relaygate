...
func RunCapture(opt Options) (string, error) {
	return dataplane.CaptureStdout(func() error { return Run(opt) })
}
...
func Run(opt Options) error {
	...
	env, err := dataplane.LoadEnv(opt.Root)
	if err != nil {
		return err
	}
	...
	check("docker", func() error {
		...
		if out, err := exec.Command("docker", "version", "--format", "{{.Server.Version}}").CombinedOutput(); err != nil {
			...
			if !dataplane.IsRoot() && (strings.Contains(msg, "permission denied") ||
	...
	check("binary", func() error {
		...
		if err != nil || st.IsDir() || st.Mode()&0o111 == 0 {
			if dataplane.LookPath("relaygate") {
				return nil
			}
	...
	if env.XDSEnabled {
		xdsPort := 18000
		if v := strings.TrimSpace(env.XDSPort); v != "" {
		...
		check("xds envoy", func() error {
			...
			if n, ok := parseDoctorStat(body, "listener_manager.lds.update_rejected"); ok && n > 0 {
				fmt.Printf("WARN: LDS 曾被拒绝 %d 次（历史累计）；若刚热更新失败请查 Envoy 日志或改用硬重启应用\n", n)
			}
			return nil
		})
	}
	...
	if opt.EnablePanel {
		check("panel login", func() error {
			if dataplane.HTTPGetOK("http://127.0.0.1:9000/login") {
				return nil
			}
	...
	if len(failures) > 0 {
		...
		for _, f := range failures {
			if strings.HasPrefix(f, ".env") || strings.HasPrefix(f, "resources") ||
				...
				strings.HasPrefix(f, "xds port") || strings.HasPrefix(f, "xds bootstrap") {
				hard++
			}
...
func dualActiveOrNLBHints(root string, env dataplane.Env) bool {
	role := strings.ToLower(strings.TrimSpace(env.PanelRole))
	if role == "standby" {
		return true
	}
...
func parseDoctorStat(statsBody, name string) (int64, bool) {
	prefix := name + ": "
