...
| 配置编辑 | `/config` | 编辑意图（落盘） |
| 配置应用 | `/apply` | 本机应用（热/硬、防火墙） |
| 运维工具 | `/ops` | 本机诊断 / 摘流 / 探测 / 防火墙 / 档位 |
| 变更历史 | `/changes` | 本机回滚 |
| 机群管理 | `/fleet` | 节点 · 按台同步 · 发布概况 · 接入 · 退役（机群页；置于本机项下、监控上） |
| 监控面板 | Grafana | 中心观测 |

**机群页**：节点列表 · 按台同步 · 发布概况 · 接入节点 · 退役节点  
**配置应用**：仅本机应用（热/硬、防火墙）；机群发布走 CLI `relaygate fleet publish`。风险说明只在各操作二次确认框。
...
| 橙 | `caution` | 热更新本机应用、单节点同步、恢复承接、应用档位 |
